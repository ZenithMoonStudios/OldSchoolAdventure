...
namespace AccelerometerHelper
{
    ...
    public sealed class AccelerometerHelper : IDisposable
    {
        ...
        /// </summary>
        private const float MaximumCalibrationTiltAngle = (float)(20.0 * Math.PI / 180.0); // 20 deg inclination from non-calibrated axis max
        ...
        /// </summary>
        private const float MaximumStabilityTiltDeltaAngle = (float)(0.5 * Math.PI / 180.0); // 0.5 deg inclination delta at max
        ...
        /// </summary>
		private Vector3 _sampleSum = new Vector3(0.0f * (float)SamplesCount, 0.0f * (float)SamplesCount, -1.0f * (float)SamplesCount); // assume start flat: -1g in z axis
        ...
        private static Vector3 AccelerometerCalibrationPersisted
        {
            get
            {
                //float x = ApplicationSettingHelper.TryGetValueWithDefault<double>(AccelerometerCalibrationKeyName + "X", 0);
                //float y = ApplicationSettingHelper.TryGetValueWithDefault<double>(AccelerometerCalibrationKeyName + "Y", 0);
                float x = 0f;
                float y = 0f;
        ...
        private void sensor_ReadingChanged(object sender, AccelerometerReadingEventArgs e)
        {
            ...
            Vector3 rawAcceleration = new Vector3((float)e.X, (float)e.Y, (float)e.Z);
